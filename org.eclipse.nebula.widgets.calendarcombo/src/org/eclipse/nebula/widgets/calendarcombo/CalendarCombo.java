...
public class CalendarCombo extends Composite {
     ...
     * @param parent Parent control
     * @param style Combo style
     * @param allowDateRange Whether to allow date range selection (note that if
     *            there is no depending CalendarCombo set you will have to deal
     *            with the date range event yourself).
     */
     ...
     * @param parent
     * @param style
     * @param dependingCombo CalendarCombo from where start date is pulled when
     *            no date has been set locally.
     */
            ...
            Listener traverseListener = new Listener() {
                public void handleEvent(Event event) {
                    if (event.detail == 16 || event.detail == 8 || event.detail == 4 || event.detail == 0) {
                        ...
                        try {
                            DateHelper.getDate(comboText, mSettings.getDateFormat());
                        } catch (Exception err) {
            ...
            mKeyDownListener = new Listener() {
                public void handleEvent(Event event) {
                    ...
                    if (event.keyCode == SWT.ARROW_DOWN) {
                        ...
                        if (isCalendarVisible()) {
                            mCalendarComposite.keyPressed(event.keyCode, event.stateMask);
                            // eat event or cursor will jump around in combo as well
                            event.doit = false;
                        }
                    }
                }
    ...
    private void setDateBasedOnComboText() {
        try {
            Date d = DateHelper.getDate(isFlat ? mFlatCombo.getText() : mCombo.getText(), mSettings.getDateFormat());
            setDate(d);
        } catch (Exception err) {
            // we don't care
        }
