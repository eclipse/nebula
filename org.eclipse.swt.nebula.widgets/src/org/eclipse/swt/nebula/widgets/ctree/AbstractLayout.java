...
public abstract class AbstractLayout extends Layout {
	 ...
	 * @param tree
	 */
	protected AbstractLayout(AbstractContainer container) {
...
//		lastColWidth = columnWidths[columnWidths.length -1];
	...
	protected Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
		if(columnWidths == null || container.isOperation(AbstractContainer.OP_NONE) || container.isOperation(AbstractContainer.OP_COLUMN_RESIZE)) {
	...
	protected void layout(Composite composite, boolean flushCache) {
		container.setOperation(AbstractContainer.OP_NONE);
	...
	protected void layoutHeader(Rectangle r) {
		...
		AbstractColumn[] ca = container.getColumns();
		...
		if(!fillers.isEmpty()) {
			...
			for(Iterator i = fillers.iterator(); i.hasNext(); ) {
				AbstractColumn col = (AbstractColumn) i.next();
				if(i.hasNext()) {
	...
	private void layoutTable(Rectangle r) {
		if(container.internalTable != null) {
			...
			if(AbstractContainer.win32 && container.sc.getHorizontalBar() != null) {
	...
	protected void opLayoutColumnResize(Rectangle r) {
		...
		r.height -= headerSize.y;
		layoutBody(r);
		if(AbstractContainer.gtk) layoutHeader(r);
		layoutItems(r);
	}
	protected void opLayoutCustom(int operation, Rectangle r) {
		AbstractItem item = (AbstractItem) container.items.get(0);
		if(item != null) {
			container.setDirtyItem(item);
		}
		container.layout();
	}
	...
	protected void opLayout() {
		...
		case AbstractContainer.OP_ADD:
			opLayoutAdd(r);
			break;
		case AbstractContainer.OP_REMOVE:
			opLayoutRemove(r);
			break;
		case AbstractContainer.OP_CELL_COLLAPSE:
			opLayoutCellCollapse(r);
			break;
		case AbstractContainer.OP_CELL_EXPAND:
			opLayoutCellExpand(r);
			break;
		case AbstractContainer.OP_COLUMN_RESIZE:
