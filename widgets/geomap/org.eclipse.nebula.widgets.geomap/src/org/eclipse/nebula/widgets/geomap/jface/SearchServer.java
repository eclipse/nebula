...
public abstract class SearchServer extends URLService {
	...
	private Object[] doSearchInternal(String search) {
        ...
        try {
            ...
            factory.newSAXParser().parse(getSearchURL(search), new DefaultHandler() {
                ...
                public void startElement(String uri, String localName, String qName, Attributes attributes) {
                	addResult(SearchServer.this.startElement(qName, path, attributes, objects));
                	path.push(qName);
                }
                public void endElement(String uri, String localName, String qName) throws SAXException {
                	addResult(SearchServer.this.endElement(qName, path, objects));
                	path.pop();
                }
                public void characters(char[] ch, int start, int length) throws SAXException {
                	String qName = path.pop();
					addResult(SearchServer.this.characters(qName, path, ch, start, length, objects));
					path.push(qName);
                }
            });
    ...
    public static class Result extends Located.Static {
        ...
        public PointD getLonLat() {
            return (Double.isNaN(lon) || Double.isNaN(lat) ? null : new PointD(lon, lat));
        }
