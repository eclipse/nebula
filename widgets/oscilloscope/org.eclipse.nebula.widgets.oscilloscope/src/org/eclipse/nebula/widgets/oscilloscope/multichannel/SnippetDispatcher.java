...
public abstract class SnippetDispatcher extends OscilloscopeDispatcher {
	...
	public void dispatch(Shell shell) {
		...
		int channels = new Random().nextInt(10) + 1;

		gilloscope = new Oscilloscope(channels, group, SWT.None);
		...
		for (int i = 0; i < channels; i++) {
			...
			if (new Random().nextInt(2) == 1)
				gilloscope.setTailSize(i, new Random().nextInt(200) + 1);
			else
				gilloscope.setTailSize(i, -1);

			int dice = new Random().nextInt(6) + 1;
			boolean boo = dice > 3 ? true : false;

			switch (dice) {
			case 1:
				gilloscope.setForeground(i, Display.getCurrent()
			...
			gilloscope.setPercentage(i, isPercentage());
			gilloscope.setTailSize(i, getTailSize());
			gilloscope.setSteady(i, isSteady(), getSteadyPosition());
			gilloscope.setFade(i, getFade());
			gilloscope.setTailFade(i, getTailFade());
			gilloscope.setConnect(i, mustConnect());
			gilloscope.setLineWidth(i, getLineWidth());
