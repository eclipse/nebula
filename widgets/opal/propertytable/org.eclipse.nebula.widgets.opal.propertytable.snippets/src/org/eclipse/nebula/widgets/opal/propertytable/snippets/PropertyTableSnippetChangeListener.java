...
public class PropertyTableSnippetChangeListener {
	...
	public static void main(final String[] args) {
		...
		button.addListener(SWT.Selection, e-> {
			final PropertyTable table = (PropertyTable) tabFolder.getSelection()[0].getControl();
	...
	private static PropertyTable buildPropertyTable(final TabFolder tabFolder, final boolean showButton, final boolean showAsCategory, final boolean showDescription) {
		...
		final Font font = SWTGraphicUtil.buildFontFrom(table, SWT.BOLD, 10);
		table.addListener(SWT.Dispose, e -> {
			SWTGraphicUtil.safeDispose(font);
		});

		table.addChangeListener(property -> {
			System.out.println("Change for " + property.getDisplayName() + ": new value is " + property.getValue());
			property.changeFont(font);
			property.changeForegroundColor(table.getDisplay().getSystemColor(SWT.COLOR_WHITE));
			property.changeBackgroundColor(table.getDisplay().getSystemColor(SWT.COLOR_RED));
		});
