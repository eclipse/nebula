...
/**
 * Interface for the data provider of trace. This gives the possibilities to
 * implement different data provider, which could have different data source or
 * data storage structure. For example: the data source could be from user
 * input, database, files, etc,. The storage structure could be array, queue,
 * circular buffer, bucket buffer, etc,.
 * <p>
 * An API like <code>
 *  public ISample[] getSamples()
 * </code> would be much easier to use by the XY Graph, but it forces the data
 ...
 * <code>synchronizes</code> on the <code>IDataProvider</code> like this to
...
public interface IDataProvider {
	 ...
	 * Get sample by index
	 * <p>
	 * <b>Synchronization:</b> Since the data might change dynamically,
	 * <code>synchronize</code> on the <code>IDataProvider</code> around calls
	 * to <code>getSize()</code> and <code>getSample()</code>.
	 *
	 * @param index
