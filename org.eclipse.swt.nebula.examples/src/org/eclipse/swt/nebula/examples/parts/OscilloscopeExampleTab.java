...
import org.eclipse.swt.nebula.examples.internal.Activator;
...
public class OscilloscopeExampleTab extends AbstractExampleTab {
	...
	public Control createControl(Composite parent) {
		...
		oscilloscope.setForeground(oscilloscope.getDisplay().getSystemColor(
				SWT.COLOR_RED));
		...
		try {
			dispatcher = new OscilloscopeDispatcher() {
				...
				public void setRandomPulse(int v) {
					...
					getOscilloscope().setValue(100 - new Random().nextInt(200));
				}
				...
				public File getActiveSoundfile() {
					if (activeSoundCombo.getSelectionIndex() == -1)
						return null;
					else if (activeSoundCombo.getItem(
							activeSoundCombo.getSelectionIndex()).equals(
							"Heartbeat")) {
						return new File(HEARTBEAT);
					} else if (activeSoundCombo.getItem(
							activeSoundCombo.getSelectionIndex())
							.equals("Beep"))
				...
				public File getInactiveSoundfile() {
					if (inactiveSoundCombo.getSelectionIndex() == -1)
						return null;
					else if (inactiveSoundCombo.getItem(
							inactiveSoundCombo.getSelectionIndex()).equals(
							"Heartbeat"))
						return new File(HEARTBEAT);
					else if (inactiveSoundCombo.getItem(
							inactiveSoundCombo.getSelectionIndex()).equals(
				...
				public Image getBackgroundImage() {
					...
					if (image1 == null) {
						...
						image1 = new Image(null,
								new ByteArrayInputStream(bytes));
					}

					if (imageCombo.getItem(imageCombo.getSelectionIndex())
							.equals("None"))
						return null;
					else if (imageCombo.getItem(imageCombo.getSelectionIndex())
							.equals("SMALL RASTER"))
						return image1;
	...
	private Control getSettings(Composite parent) {
...

		{
			...
			grpSpeed.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false,
					false, 1, 1));
			...
			serviceActive.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					if (serviceActive.getSelection())
						oscilloscope.setForeground(oscilloscope.getDisplay()
...

		{
			...
			lblBackgroundImage.setLayoutData(new GridData(SWT.RIGHT,
...

		{
			...
			grpTailsize.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false,
					false, 4, 1));
			...
			lblIfTailsizemaxAnd.setLayoutData(new GridData(SWT.LEFT,
...

		{
			...
			activeSoundCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
					true, false, 1, 1));
			...
			inactiveSoundCombo.setItems(new String[] { "Heartbeat", "Beep",
					"Flatline" });
			inactiveSoundCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
					true, false, 1, 1));
		}
